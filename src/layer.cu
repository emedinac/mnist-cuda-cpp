...
void Layer::update_parameters(float learning_rate)
{
...
int Layer::load_pretrained_model()
{
...
int Softmax::get_acc(Blob<float> *target)
{
...
int Layer::get_acc(Blob<float> *target)
{
	assert("No Loss layer cannot estimate acc." && false);
	return EXIT_FAILURE;
}