...
int main(int argc, char* argv[])
{
    ...
    model.add_layer(new Pooling("pool", 2, 0, 2, CUDNN_POOLING_MAX));
    model.add_layer(new Dense("dense1", 250));
    ...
    while (step < num_steps_train)
    {
        ...
        tp_count += model.get_acc(train_target);
        ...
        if (step % monitoring_step == 0)
        {
            ...
            float acc =  100.f * tp_count / monitoring_step / batch_size_train;
    ...
    while (step < num_steps_test)
    {
        ...
        tp_count += model.get_acc(test_target);
    ...
    // step 4. calculate loss and acc
    float loss = model.loss(test_target);
    float acc = 100.f * tp_count / num_steps_test / batch_size_test;

    std::cout << "loss: " << std::setw(4) << loss << ", acc: " << acc << "%" << std::endl;
